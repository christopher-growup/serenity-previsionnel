...
describe('calculerTableauEmprunt', () => {
  it('returns empty array for zero amount', () => {
    expect(calculerTableauEmprunt({ description: '', montant: 0, dureeMois: 12, tauxAnnuel: 5 })).toEqual([]);
  });

  it('computes correct total repayment', () => {
    const tableau = calculerTableauEmprunt({ description: 'Pret', montant: 6500, dureeMois: 60, tauxAnnuel: 5 });
  ...
  it('first payment has more interest than last', () => {
    const tableau = calculerTableauEmprunt({ description: 'Pret', montant: 10000, dureeMois: 36, tauxAnnuel: 6 });
    expect(tableau[0].interets).toBeGreaterThan(tableau[35].interets);
  });

  it('monthly payment is constant', () => {
    const tableau = calculerTableauEmprunt({ description: 'Pret', montant: 10000, dureeMois: 24, tauxAnnuel: 4 });
